...
import com.dakers.lambda.stlc.VarRule.AbstRule
...
class AbstRuleTest extends FlatSpec with Matchers with STNotation with UTNotation {
  ...
  "x : T|- x : T " should "imply that |- /|x.x : T -> T" in {
    ...
    AbstRule(j1) should be(Some(stlc.Judgement(SimplyTypedDerivationContext(), /|("x", "x") :| "T" ->: "T".tv)))
  }

  //Abstracting over the same variable
  "y : U, x : T|- x : T " should "imply that |- /|x.x : T -> T" in {
    val list: ListBuffer[STStatement] = ListBuffer[STStatement]()
    list += "y" :| "U"
    list += "x" :| "T"
    val ctx = SimplyTypedDerivationContext(list.toList)
    val j1 = stlc.Judgement(ctx, "x" :| "T")
    AbstRule(j1) should be(Some(stlc.Judgement(SimplyTypedDerivationContext(List("y" :| "U")), /|("x", "x") :| "T" ->: "T".tv)))
  }
  ...
  "x : T, y : T|- x : T " should "imply that |- /|y.x : T -> T" in {
    ...
    val ctx = SimplyTypedDerivationContext(list.toList)
    val j1 = stlc.Judgement(ctx, "x" :| "T")
    AbstRule(j1) should be(Some(stlc.Judgement(SimplyTypedDerivationContext(List("x" :| "T")), /|("y", "x") :| "T" ->: "T".tv)))
  }
  ...
  "x : T, y : U|- x : T " should "imply that |- /|y.x : U -> T" in {
    ...
    val ctx = SimplyTypedDerivationContext(list.toList)
    val j1 = stlc.Judgement(ctx, "x" :| "T")
    AbstRule(j1) should be(Some(stlc.Judgement(SimplyTypedDerivationContext(List("x" :| "T")), /|("y", "x") :| "U" ->: "T".tv)))
  }

  "Abstracting over an application: x : T|- xy : T " should "imply that |- /|x.xy : T -> T" in {
    val ctx = SimplyTypedDerivationContext(List("x" :| "T".tv))
    val j1 = stlc.Judgement(ctx, "x" * "y" :| "T")
    AbstRule(j1) should be(Some(stlc.Judgement(SimplyTypedDerivationContext(), /|("x", "x" * "y") :| "T" ->: "T".tv)))
  }
  ...
  "Abstracting over an application: y : U, x : T|- xz : T " should "imply that |- /|x.xz : T -> T" in {
    ...
    list += "x" :| "T"
    val ctx = SimplyTypedDerivationContext(list.toList)
    val j1 = stlc.Judgement(ctx, "x" * "z" :| "T")
    AbstRule(j1) should be(Some(stlc.Judgement(SimplyTypedDerivationContext(List("y" :| "U")), /|("x", "x" * "z") :| "T" ->: "T".tv)))
  }
  ...
  "Abstracting over an application: x : T, y : T|- xz : T " should "imply that |- /|y.xz : T -> T" in {
    ...
    val ctx = SimplyTypedDerivationContext(list.toList)
    val j1 = stlc.Judgement(ctx, "x" * "z" :| "T")
    AbstRule(j1) should be(Some(stlc.Judgement(SimplyTypedDerivationContext(List("x" :| "T")), /|("y", "x" * "z") :| "T" ->: "T".tv)))
  }
  ...
  "Abstracting over an application: x : T, y : U|- x : T " should "imply that |- /|y.x : U -> T" in {
    ...
    val ctx = SimplyTypedDerivationContext(list.toList)
    val j1 = stlc.Judgement(ctx, "x" * "y" :| "T")
    AbstRule(j1) should be(Some(stlc.Judgement(SimplyTypedDerivationContext(List("x" :| "T")), /|("y", "x" * "y") :| "U" ->: "T".tv)))
  }
  ...
  "Abstracting over an abstraction: x : T, z : T|- /|x.x : T -> T " should "imply that |- /|z./|x.x : T -> T -> T" in {
    ...
    val j1 = stlc.Judgement(ctx, /|("x", "x") :| "T" ->: "T".tv)
    AbstRule(j1) should be(Some(stlc.Judgement(SimplyTypedDerivationContext(List("x" :| "T".tv)), /|("z", /|("x", "x")) :| "T".tv ->: "T" ->: "T".tv)))
  }
  ...
  "Abstracting over an abstraction: x : T, z : U|- /|x.x : T -> T " should "imply that |- /|z./|x.x : U -> T -> T" in {
    ...
    val j1 = stlc.Judgement(ctx, /|("x", "x") :| "T" ->: "T".tv)
    AbstRule(j1) should be(Some(stlc.Judgement(SimplyTypedDerivationContext(List("x" :| "T".tv)), /|("z", /|("x", "x")) :| "U".tv ->: "T" ->: "T".tv)))
  }
  ...
  "Abstraction over a judgement with an empty context" should "return None" in {
    ...
    AbstRule(stlc.Judgement(ctx, /|("x", "x") :| "T".tv ->: "T".tv)) should be(None)
  }
