...
class ApplRule2Test extends FlatSpec with Matchers with UTNotation with STNotation with L2Notation {
  ...
  "Applying two terms with different contexts" should "return None" in {
    val g1 = L2Context()
    val g2 = L2Context(List(L2Statement("x", "X".tv)))
    Appl2(Judgement(g1, L2Statement("x", "x")), Judgement(g2, L2Statement("x", "x"))) should be(None)
  }

  "If the first term is not a pi type, Appl2" should "return None" in {
    val g1 = L2Context(List(L2Statement("x", "X".tv)))
    Appl2(Judgement(g1, L2Statement("x", "x")), Judgement(g1, L2Statement("x", "x"))) should be(None)
  }
  ...
  "Given |- (|^|A:*) (/|x:A)x[A:=B]" should "return Some(/|x:B)" in {
    ...
    Appl2(Judgement(g, stmt1), Judgement(g, stmt2)) should be(Some(Judgement(g, L2Statement(ut1 * ut2, "T1".tv))))
  }
  ...
  "Given |- (|^|A:*) (/|x:A)xy[A:=B->C]" should "return Some(/|x x:B->C)" in {
    ...
    val g = L2Context(List())
    Appl2(Judgement(g, stmt1), Judgement(g, stmt2)) should be(Some(Judgement(g, L2Statement(ut1 * ut2, ArrType2("B", "C")))))
  }
  ...
  "Given |- (|^|A:*) (/|x:A)x[A:=π(b, b)]" should "return Some(/|x x:B->C)" in {
    ...
    val g = L2Context(List())
    Appl2(Judgement(g, stmt1), Judgement(g, stmt2)) should be(Some(Judgement(g, L2Statement(ut1 * ut2, piB))))
  }
