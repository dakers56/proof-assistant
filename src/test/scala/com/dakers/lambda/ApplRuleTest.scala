...
import com.dakers.lambda.VarRule.ApplRule
...
class ApplRuleTest extends FlatSpec with Matchers with UTNotation with STNotation {
  ...
  "If x : X -> Y and y : X, ApplRule(|- x: X, |- y : X -> Y" should "be of type Y" in {
    val st1: Statement = "x" :| "S".tv ->: "T".tv
    val st2 = "y" :| "S"
    val ctx = SimplyTypedDerivationContext(List(st1, st2))
    ApplRule(Judgement(ctx, st1), Judgement(ctx, st2)) should be(Some(Judgement(ctx, "x" * "y" :| "T")))
  }

  // Variable to a function type
  "If x : S -> T -> U and y:S , ApplRule(|- x: S -> T -> U, |- y : S " should "be of type T -> U" in {
    val st1: Statement = "x" :| "S".tv ->: "T".tv ->: "U".tv
    val st2 = "y" :| "S"
    val ctx = SimplyTypedDerivationContext(List(st1, st2))
    ApplRule(Judgement(ctx, st1), Judgement(ctx, st2)) should be(Some(Judgement(ctx, "x" * "y" :| "T" ->: "U".tv)))
  }
  ...
  "If x: (S -> T) -> U and y: S -> T , ApplRule(|- x: (S -> T) -> U, |- y : S -> T " should "be of type U" in {
    ...
    val st2 = "y" :| "S" ->: "T".tv
    val ctx = SimplyTypedDerivationContext(List(st1, st2))
    ApplRule(Judgement(ctx, st1), Judgement(ctx, st2)) should be(Some(Judgement(ctx, "x" * "y" :| "U")))
  }

  // Associativity: function type to a function type
  "If x: (S -> T) -> U -> V and y: S -> T , ApplRule(|- x: (S -> T) -> U -> V, |- y : S -> T " should "be of type U -> V" in {
    val st1: Statement = "x" :| ("S".tv ->: "T".tv) ->: "U".tv ->: "V".tv
    val st2 = "y" :| "S" ->: "T".tv
    val ctx = SimplyTypedDerivationContext(List(st1, st2))
    ApplRule(Judgement(ctx, st1), Judgement(ctx, st2)) should be(Some(Judgement(ctx, "x" * "y" :| "U" ->: "V".tv)))
  }

  //Application of  a variable to an abstraction term
  "ApplRule(|- |\\x.x : T->U, |- y : T) " should " imply |- (|\\x.x)y : U" in {
    val st1: Statement = /|("x", "x") :| "T".tv ->: "U".tv
    val st2 = "y" :| "T"
    val ctx = SimplyTypedDerivationContext(List(st1, st2))
    ApplRule(Judgement(ctx, st1), Judgement(ctx, st2)) should be(Some(Judgement(ctx, /|("x", "x") * "y" :| "U")))
  }

  //Application of  an abstraction term to a variable term
  "ApplRule(|- |\\x.x : S->T->U, |- y : S ) " should " imply |- (|\\x.x)y : T -> U" in {
    val st1: Statement = /|("x", "x") :| "S" ->: "T" ->: "U".tv
    val st2 = "y" :| "S"
    val ctx = SimplyTypedDerivationContext(List(st1, st2))
    ApplRule(Judgement(ctx, st1), Judgement(ctx, st2)) should be(Some(Judgement(ctx, /|("x", "x") * "y" :| "T".tv ->: "U".tv)))
  }

  //Negative cases

  "Applying a variable to a variable " should " fail" in {
    val st1: Statement = "x" :| "T"
    val st2 = "y" :| "S"
    val ctx = SimplyTypedDerivationContext(List(st1, st2))
    ApplRule(Judgement(ctx, st1), Judgement(ctx, st2)) should be(None)
  }
  ...
  "Different contexts and a valid application xy where x : S -> T, y: S" should "fail" in {
    ...
    ApplRule(Judgement(ctx1, st1), Judgement(ctx2, st2)) should be(None)
  }
