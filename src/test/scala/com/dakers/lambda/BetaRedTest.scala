...
//  }
//
//  s"$binder x $sep ($binder y $sep (yz)" should "have no redexes" in {
//    BetaRed.redexes(lam("/|x./|y.(yz)")) should
//      be(List())
//  }
//
//  s"$binder x $sep ($binder y $sep (yx)z)" should "have two redexes" in {
//    BetaRed.redexes(lam("(/|x.((/|y.yx))z)")) should
//      be(List(lam("(/|x.(zx))v"), lam("/|y.(yz)")))
//  }
